...
function StatusServidor({ status, historico, eventos }) {
    ...
    <>
      ...
      <div className="protheus-metrics-row">
        <div className="protheus-metric">
          <span className="protheus-metric-value">{status.latencia_ms !== null && status.latencia_ms !== undefined ? `${status.latencia_ms.toFixed(1)}ms` : '—'}</span>
      ...
      <div className="protheus-uptime-row">
        <div className="protheus-uptime-item">
          <span className="protheus-uptime-value">{status.uptime_24h !== null && status.uptime_24h !== undefined ? `${status.uptime_24h}%` : '—'}</span>
          <span className="protheus-uptime-label">24h</span>
        </div>
        <div className="protheus-uptime-item">
          <span className="protheus-uptime-value">{status.uptime_7d !== null && status.uptime_7d !== undefined ? `${status.uptime_7d}%` : '—'}</span>
          <span className="protheus-uptime-label">7 dias</span>
        </div>
        <div className="protheus-uptime-item">
          <span className="protheus-uptime-value">{status.uptime_30d !== null && status.uptime_30d !== undefined ? `${status.uptime_30d}%` : '—'}</span>
